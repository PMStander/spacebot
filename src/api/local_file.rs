...
//!   2. Path must canonicalise to a location under any registered agent workspace.
...
pub(super) async fn serve_local_file(
    ...
    // 4. Must be under any registered agent workspace.
    let workspaces = state.agent_workspaces.load();
    let under_workspace = workspaces
        .values()
