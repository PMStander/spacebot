...
impl SpacebotModel {
    ...
    async fn attempt_completion(
        ...
        if provider_id == "zai-coding-plan" || provider_id == "zhipu" || provider_id == "zhipu-sub"
        {
            let display_name = match provider_id {
                "zhipu" => "Z.AI (GLM)",
                "zhipu-sub" => "Z.AI Subscription",
                _ => "Z.AI Coding Plan",
            };
            let endpoint = format!(
                "{}/chat/completions",
...
impl SpacebotModel {
    ...
    #[allow(dead_code)] // kept for providers that require auth — complements call_openai_compatible_with_optional_auth
...
#[allow(dead_code)] // kept for future Ollama provider support
